 ...
 */
function mapRecordToProduct(record: any, lang: string = 'en'): Product {
...
const mapRecordToProductDefault = (record: any): Product => mapRecordToProduct(record, 'en');
    ...
    async getProducts(
        ...
        try {
            ...
            if (filters?.search) {
                ...
                // Use TDD fields: name_en, name_fr, description_en, description_fr
                filterParts.push(`(name_en ~ "${searchTerm}" || name_fr ~ "${searchTerm}" || description_en ~ "${searchTerm}" || description_fr ~ "${searchTerm}")`);
            }
