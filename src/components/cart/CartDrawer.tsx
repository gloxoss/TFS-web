...
function CartSynchronizer() {
  ...
  useEffect(() => {
    // Only sync once per user session to avoid overwriting local changes
    if (user && !hasSyncedRef.current) {
      hasSyncedRef.current = true
      console.log('[CartSynchronizer] Fetching server cart for user:', user.email)

      getUserCart().then((result) => {
        console.log('[CartSynchronizer] Server cart result:', result.success, result.items?.length, 'items')
        if (result.success && result.items && result.items.length > 0) {
          setItems(result.items)
        }
      }).catch((error) => {
        console.error('[CartSynchronizer] Failed to sync cart:', error)
      })
    }
...
export function CartDrawer({ lng }: CartDrawerProps) {
  ...
  // Handle wheel events manually to ensure scroll works
  ...
  // Simpler body scroll lock - just hide overflow
  useEffect(() => {
    if (isOpen) {
          ...
          <div className="fixed inset-0 overflow-hidden">
            <div className="absolute inset-0 overflow-hidden">
              <div className="fixed inset-y-0 right-0 flex max-w-full pl-10">
                ...
                >
                  <DialogPanel className="w-screen max-w-md h-full">
                    <div className="flex h-full flex-col bg-zinc-950 border-l border-zinc-800 shadow-xl overflow-hidden">
                      {/* Header */}
                      ...
                      {/* Content - min-h-0 is critical for flex scroll */}
                      <div
                        ref={contentRef}
                        onWheel={handleWheel}
