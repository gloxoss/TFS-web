...
const CATEGORY_ICON_MAP = {
    [CategoryEnum.APPLICATION]: Layers,
    [CategoryEnum.AI]: Wand2,
    [CategoryEnum.MARKETING]: TrendingUp,
    [CategoryEnum.ECOMMERCE]: ShoppingCart,
    [CategoryEnum.CHARTS]: PieChart,
};
...
export default function CommandMenuWithCategories() {
    ...
    // Use a default value for isMobile to avoid null issues during SSR or initial render
    const isMobile = useMediaQuery("(max-width: 650px)") ?? false;
    ...
    useEffect(() => {
        const onKeyDown = (e: KeyboardEvent) => {
            ...
            if (e?.key?.toLowerCase() === "k" && (e as any)[hotkey]) {
                e.preventDefault();
    ...
    const renderSearchItem = useCallback(
        (item: SearchResultItem, index: number, isRecent: boolean) => {
            ...
            const LinkIcon = CATEGORY_ICON_MAP[item.category as CategoryEnum];
            ...
            const content = (
                ...
                >
                    <div className={isMobile ? slots.leftWrapperOnMobile() : slots.leftWrapper()}>
                        {item.category && (
                            <LinkIcon className={slots.leftIcon()} size={20} />
                        )}
            ...
            >
                {CATEGORIES.map((item) => (
                    <ListboxItem
                        ...
                        startContent={<item.icon className="text-default-400" size={20} />}
    ...
    return (
        <>
            ...
            >
                ...
                >
                    <Search className="text-default-400 [&>g]:stroke-[2px]" size={18} />
                    Toggle command menu
                </Button>
            </Tooltip>
            <Modal
                ...
                <ModalContent>
                    <Command className={slots.base()} label="Quick search command" shouldFilter={false}>
                        ...
                        <div className="relative grid grid-cols-12 gap-4">
                            ...
                            <div
                                ref={listRef}
                                className={cn(slots.listScroll(), {
                                    "col-span-8": !isMobile && isEmpty(query),
                                    "col-span-12": isMobile || !isEmpty(query),
                                })}
                            >
                                {!isEmpty(query) && (
                                    <Command.List className={slots.list()}>
                                        {items.map((item, index) => renderSearchItem(item, index, false))}
                                        {items.length === 0 && (
