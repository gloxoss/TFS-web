...
async function seedImages() {
    try {
        ...
        for (const item of DATA) {
            ...
            const tempFile = path.resolve(`temp_${slug}.jpg`);
            ...
            try {
                ...
                try {
                    console.log('   -> Attempting placeholder...');
                    const phUrl = `https://placehold.co/600x400/png?text=${encodeURIComponent(item.name)}`;
                    const phCmd = `cmd /c curl -L -o "${tempFile}" "${phUrl}"`;
                    execSync(phCmd, { stdio: 'ignore' });

                    const phFormData = new FormData();
                    const phBuffer = fs.readFileSync(tempFile);
                    const phBlob = new Blob([phBuffer]);
                    ...
                    console.error('   -> Placeholder FAILED too.');
                }
            } finally {
